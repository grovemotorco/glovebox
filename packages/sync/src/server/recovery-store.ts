 ...
 * are explicit JSON references to durable content or intent metadata. Deviation from the
...
export interface WorkspaceRecoveryRecord {
  ...
  /** JSON payload with structured recovery metadata. */
  payload: string
  createdAt: number
...
export class WorkspaceRecoveryStore {
  ...
  get(recordId: string): WorkspaceRecoveryRecord | null {
    const row = this.#sql
      .exec('SELECT * FROM workspace_recovery_records WHERE record_id = ? LIMIT 1', recordId)
      .toArray()[0]
    if (!row) return null
    return {
      recordId: row.record_id as string,
