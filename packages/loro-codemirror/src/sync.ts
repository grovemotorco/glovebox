  ...
  onRemoteUpdate = (e: LoroEventBatch) => {
    ...
    if (e.by === 'import') {
      ...
      for (const { diff, target } of e.events) {
        // A batch can interleave events from several containers; events
        // that are not this document's text container are skipped — never
        // aborted on, or a later text event in the same batch is lost.
